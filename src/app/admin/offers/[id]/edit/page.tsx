        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {offers.map((offer: any) => (
              <div key={offer.id} style={{ background: "#111", border: `1px solid ${offer.active ? "#FFD70030" : "#222"}`, borderRadius: "12px", padding: "20px 24px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                  ...
                  <div style={{ fontSize: "22px", fontWeight: 800, color: "#FFD700", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", gap: "8px", marginTop: "16px", paddingTop: "16px", borderTop: "1px solid #1a1a1a", flexWrap: "wrap" }}>
                  <Link
                    href={`/en/spin/v1`}
                    target="_blank"
                    style={{ padding: "7px 14px", background: "#1a1a1a", border: "1px solid #333", borderRadius: "6px", color: "#888", fontSize: "12px", textDecoration: "none" }}
                  >
                    Preview
                  </Link>
                  <Link
                    href={`/admin/offers/${offer.id}/edit`}
                    style={{ padding: "7px 14px", background: "#1a1a00", border: "1px solid #FFD70050", borderRadius: "6px", color: "#FFD700", fontSize: "12px", textDecoration: "none" }}
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => toggleOffer(offer.id)}
                    style={{ padding: "7px 14px", background: offer.active ? "#1a0000" : "#001a00", border: `1px solid ${offer.active ? "#FF450050" : "#22C55E50"}`, borderRadius: "6px", color: offer.active ? "#FF4500" : "#22C55E", fontSize: "12px", cursor: "pointer" }}
                  >
                    {offer.active ? "Pause" : "Activate"}
                  </button>
                  <button
                    onClick={() => deleteOffer(offer.id)}
                    style={{ padding: "7px 14px", background: "#1a0000", border: "1px solid #ff000030", borderRadius: "6px", color: "#ff4444", fontSize: "12px", cursor: "pointer", marginLeft: "auto" }}
                  >
                    Delete
                  </button>
                </div>
